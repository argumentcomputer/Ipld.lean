...
impl Multibase {
  ...
  fn log2_base(&self) -> u64 {
    (self.base() as f32).log2().trunc() as u64
  }
  ...
  fn valid_digit(&self, c: char) -> bool {
    self.read(c) != None
  }

  // Checks if all characters in a given string are contained in the alpha string
  ...
  pub fn encode(&self, mut input: Vec<u8>) -> String {
    ...
    // Converts a big-endian list of bytes into u128, then into numeric string form
    // Seems like a compression operation
    let n = read_be_u128(&input).unwrap();
    let mut encode_str = n.to_string();
    let char_pad = (byte_pad * 8) / log;
    if rfc {
      encode_str = encode_str.drain(..char_pad-1).collect();
      if self.pad {
...
// Converts an unsized slice into a 16-element array, then reads it into big-endian u128 
